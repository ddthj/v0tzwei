...
def defaultPD(agent, local, direction = 0):
    ...
    up =  agent.me.matrix.dot(Vector3(0,0,1.0))
...
def flip(agent,local):
    #dodges towards a local coordinate
    pitch = -sign(local[0]) if abs(agent.me.rvel[1]) < 1.0 else -sign(agent.me.rvel[1])
    if not agent.me.airborne: #this is the e
        agent.c.jump = True
        agent.sinceJump = 0
...
def defaultThrottle(agent,target_speed,agent_speed,direction=1):
    ...
    if final >0.0:
        if abs(final) < 450 and target_speed < 1410:
            agent.c.boost = False
        else:
            agent.c.boost = True
